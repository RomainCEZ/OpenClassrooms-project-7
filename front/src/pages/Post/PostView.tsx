        ...
        <section>
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col mt-2 p-2 sm:px-5 rounded min-h-80 h-fit bg-white border border-indigo-900 divide-blue-900 divide-y-2">
                    <h2 className="text-xl font-semibold p-2 sm:px-0">
                        {post.title}
                    </h2>
                    <div className="flex w-full p-2">
                        {post.imageUrl && (
                            <img src={post.imageUrl} className="pt-3 w-full" />
                        )}
                        <Editor
                            readOnly
                            toolbarHidden
            ...
            {(user.id === post.authorId || user.role === "admin") && (
                <div className="flex justify-end items-center p-2 gap-2">
                    <BlueButton path={`/post/${id}/edit`}>Éditer</BlueButton>
                    <Link
                        to="/"
                        className="flex justify-center p-2 px-6 mb-3 rounded bg-red-900 text-white"
            ...
            <Suspense fallback={<CommentLoader />}>
                <Comment title={post.title} author={post.author} />
            </Suspense>
        </section>
    );
}
